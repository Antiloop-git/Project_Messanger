...
class TestServer(unittest.TestCase):
    ...
    def test_wrong_action(self):
        ...
        self.assertEqual(process_client_message(
            {ACTION: 'Wrong', TIME: time.asctime(), USER: {ACCOUNT_NAME: 'Guest'}}), self.err_dict)
    ...
    def test_no_user(self):
        ...
        self.assertEqual(process_client_message(
            {ACTION: PRESENCE, TIME: time.ctime()}), self.err_dict)

    def test_unknown_user(self):
        """Ошибка - не Guest"""
        self.assertEqual(process_client_message(
            {ACTION: PRESENCE, TIME: time.ctime(), USER: {ACCOUNT_NAME: 'Guest1'}}), self.err_dict)

    def test_ok_check(self):
        """Корректный запрос"""
        self.assertEqual(process_client_message(
            {ACTION: PRESENCE, TIME: time.ctime(), USER: {ACCOUNT_NAME: 'Guest'}}), self.ok_dict)
